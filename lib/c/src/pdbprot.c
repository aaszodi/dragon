...
/* ANSI C, IRIX 5.3, 2. Aug. 1996. Andris Aszodi */
 ...
 * calloc(0,...) is a Nop (21-Apr-1998.)
 */
#define G_MALLOC(PTR, TYPE) (PTR)=(TYPE*) malloc(sizeof(TYPE))
#define G_CALLOC(PTR, TYPE, SIZE) \
    if (SIZE) (PTR)=(TYPE*) calloc((SIZE), sizeof(TYPE))
#define G_REALLOC(PTR, TYPE, SIZE) \
    if ((PTR)!=NULL || (SIZE)) (PTR)=(TYPE*) realloc((PTR), (SIZE)*sizeof(TYPE))
...
static long set_pdbdate(Pdbentry_ *Entry);
 ...
 * If Ca!=0 then only the C-alpha atoms are read in;
...
Pdbentry_ *get_pdb(const char *Pdbfn, int Ca,int Strict)
{
    ...
    char Oldrid,Oldchain, Creatchn,Endf,Protein,Calpha,Thisca, Model, Ter;       
	...
	if (Creatchn)  /* start new chain */
	{
	    ...
	    i=Aano=0; Oldrid=' '; Oldchain=Line[21]; Oldaano=-9999;
	    Protein=0; Calpha=1;
	}
	...
	char *Remarks[], int Remno)
{
     ...
     * than bonds and is redundant. Atom numbers are supposed 
     * to run from 1 (0 indicates "no bond").
     */
    G_CALLOC(Hb3s, Hb3_, 2*Hbno);   /* worst case: 2 records per Hbond_ */
	...
	for (j=0; j<Hb3no; j++)
	{
	    ...
	    if (Acc==Hb3s[j].At && !Hb3s[j].Don)
	    {	
		Hb3s[j].Don=Don; Dside++;
		continue;
	    }
	    
	    /* join to j-th on the acceptor side */
	    if (Don==Hb3s[j].At && !Hb3s[j].Acc)
	    {	
		Hb3s[j].Acc=Acc; Aside++;
	    }
	}
	...
	sprintf(Card, "CONECT %4d", Hb3s[i].At);
	memset(Card+11, ' ', 20);
	if (!Hb3s[i].Acc)
	    memset(Card+31, ' ', 15);
...
/* set_pdbdate(): checks the Data field of the PDB structure
 * pointed to by Entry. If the Data field contains a valid date
 * in the format "DD-MMM-YY" then no change is made and 0
 * is returned. If Data doesn't contain a valid date string, 
 * then the current date will be used to fill it in and
 * a non-0 value (the current system time) is returned.
 */
static long set_pdbdate(Pdbentry_ *Entry)
{
    ...
    /* valid date? */
    if (strlen(Entry->Date)==9 &&
	3==sscanf(Entry->Date, "%2d-%3s-%2d", &Day, Month, &Year) &&
	1<=Day && Day<=31 &&
	NULL!=strstr(Months, Month) &&
	0<=Year && Year<=99)
	    return(0);
    
    /* invalid date, set correct value */
    Curtime=time(NULL);
    Tmp=localtime(&Curtime);
    strncpy(Month, Months+3*(Tmp->tm_mon), 3);
    sprintf(Entry->Date, "%02d-%3s-%02d", Tmp->tm_mday, Month, Tmp->tm_year);
    return(Curtime);
}
/* END of set_pdbdate() */
